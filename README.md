...
if the claim is later proven false, the stake is **slashed**. Private (TEE) + accountable (economic).
...
yarn test                    # 24 contract unit tests
...
```bash
yarn demo                    # unattended: happy path + fraud path, live on Coston2 (~5 min, 2 FDC rounds)
yarn service                 # http://localhost:7900 — 3-view UI (issue / verify / fraud)
```
