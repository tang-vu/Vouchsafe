...
| `src/SolvencyVerifier.sol` | Core. `recordSolvency(claim, teeSig, fdcProof)` verifies the TEE signature (`ecrecover == teeAddress` over a chain+verifier-bound EIP-191 digest), verifies the FDC Web2Json proof, binds attested reserves to `reservesCommitment`, gates on stake, records, locks stake. `raiseFraud(id, liabilities, salt, fdcProof)` = permissionless evidence-based slashing. |
